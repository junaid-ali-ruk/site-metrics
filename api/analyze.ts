...
interface VitalData { value: number; score: number; display: string; }
interface AuditItemData { title: string; score: number | null; displayValue?: string; }
...
function lighthouseScore(value: number, p10: number, median: number): number {
  ...
  const sigma = (Math.log(median) - Math.log(p10)) / 1.2816;
...
interface SubResources {
  totalTransferKB: number; jsTransferKB: number; cssTransferKB: number;
  jsCount: number; cssCount: number; imgCount: number; fontCount: number;
  thirdPartyCount: number; renderBlocking: number;
}
...
async function analyzeSubResources(html: string, baseUrl: string, origin: string): Promise<SubResources> {
  ...
  result.renderBlocking = blockingScripts + allCss.length;
  ...
  const sizes = await Promise.all([...scriptSrcs.slice(0, 8).map(fetchSize), ...allCss.slice(0, 5).map(fetchSize)]);
  const jsSizes = sizes.slice(0, Math.min(8, scriptSrcs.length));
  const cssSizes = sizes.slice(jsSizes.length);
...
async function analyze(rawUrl: string, strategy: string) {
  ...
  const { res, html, ttfb: ttfbMs } = await fetchWithTiming(url, ua);
  ...
  const noServerLeak = !serverHeader || !/\d+\.\d+/.test(serverHeader);
  ...
  return {
    url: finalUrl, requestedUrl: url, timestamp: new Date().toISOString(), strategy,
    engine: "sitemetrics",
